                ...
                {historyList.length === 0 ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "25px" }}>
                        {historyList.map((item) => (
                            <div key={item.history_id} style={{ background: "white", borderRadius: "12px", overflow: "hidden", boxShadow: "0 4px 15px rgba(0,0,0,0.06)", border: "1px solid #eee", transition: "transform 0.2s" }} onMouseOver={e => e.currentTarget.style.transform = 'translateY(-5px)'} onMouseOut={e => e.currentTarget.style.transform = 'translateY(0)'}>
                                ...
                                <div style={{ padding: "15px" }}>
                                    <h3 style={{ margin: "0 0 10px 0", fontSize: "1.1rem", color: "#333", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={item.title}>{item.title}</h3>            
                                    ...
                                    <Link to={`/${item.work_type === 'novel' ? 'novel' : 'manga'}/${item.work_id}/chapters/${item.chapter_id}`} style={{ textDecoration: "none" }}>
